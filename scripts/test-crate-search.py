#!/usr/bin/env python3
"""Actual MCP crate search modes, filters and native fallback; no provisioning."""
...
result=subprocess.run(['cargo','test','-p','rust-engineering-mcp','--features','local','--test','crate_search','--locked','--offline','--no-run','--message-format=json'],env=env,check=True,stdout=subprocess.PIPE,text=True)
binaries=[]
...
if len(binaries)!=1: sys.exit('Exact MCP crate search test binary missing')
...
print('PASS MCP crate search, real E5/native rankings, shared generation and explicit fallback under network deny')
