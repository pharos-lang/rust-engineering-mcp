#!/usr/bin/env python3
"""Actual MCP paged SQLite inspection and generation-bound continuation; no provisioning."""
...
result=subprocess.run(['cargo','test','-p','rust-engineering-mcp','--features','local','--test','crate_inspect','--locked','--offline','--no-run','--message-format=json'],env=env,check=True,stdout=subprocess.PIPE,text=True)
binaries=[]
...
if len(binaries)!=1: sys.exit('Exact MCP crate inspection test binary missing')
...
print('PASS MCP paged inspection, exact SQLite facts, unknown metadata and generation-bound continuation under network deny')
