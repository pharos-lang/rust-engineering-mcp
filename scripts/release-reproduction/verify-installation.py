...
def main():
    ...
    for name in ('inventory.json', 'THIRD_PARTY_NOTICES.candidate.txt'):
        copy(ROOT / 'scripts/release-reproduction/notices-0.1.0' / name, assets / 'notices' / name)
    for source in sorted((ROOT / 'licenses/upstream').rglob('*')):
        if source.is_file():
            copy(source, assets / 'notices/upstream-licenses' /
                 source.relative_to(ROOT / 'licenses/upstream'))
