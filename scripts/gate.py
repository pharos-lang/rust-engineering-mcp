...
def main():
    ...
    try:
        ...
        run('codex-qualifier-tests',[sys.executable,'-B','scripts/test-codex-model-qualifier.py'],
            require_test_groups=True)
        run('m4-client-harness-tests',[sys.executable,'-B','scripts/test-m4-clients-unit.py'],require_test_groups=True)
        run('m4-safety-harness-tests',[sys.executable,'-B','scripts/test-m4-safety-harnesses.py'],require_test_groups=True)
        ...
        if args.mode=='full':
            ...
            run('m4-tampered-plugin',[sys.executable,'-B','scripts/test-m4-tampered-plugin.py','--server','target/debug/rust-engineering-mcp'])
            run('m4-inventory',[sys.executable,'-B','scripts/test-m4-inventory.py'])
            run('m4-runtime',[sys.executable,'-B','scripts/test-m4-runtime.py'])
