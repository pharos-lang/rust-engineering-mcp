#!/usr/bin/env python3
"""Actual MCP catalog status and native component independence; no provisioning."""
...
result=subprocess.run(['cargo','test','-p','rust-engineering-mcp','--features','local','--test','catalog_status','--locked','--offline','--no-run','--message-format=json'],env=env,check=True,stdout=subprocess.PIPE,text=True)
binaries=[]
...
if len(binaries)!=1: sys.exit('Exact MCP catalog status test binary missing')
...
with tempfile.TemporaryDirectory(prefix='rust-mcp-catalog-gate-') as scratch:
    ...
    for args,expected in [([],2),(['--ignored'],1)]:
...
print('PASS MCP catalog status, real E5/native index, immutable session, corrupted index independence under network deny')
