...
Miri rejects native diagnostic producers (build scripts, proc macros and custom
harnesses) and source Cargo/nextest/toolchain configuration. It uses fixed isolation
flags, muted interpreted output and bounded JUnit classification. A clean run is
limited to selected tests, not proof that a project is free of UB. Unsafe scanning
reports syntax with explicit cfg/macro/generated-source limits, not a safety score.
Missing/stale/omitted supply evidence and suppressions never create a clean result.
Host policy requires a pinned digest; suppression identity, owner, reason and
expiry remain auditable. Source URLs and guest diagnostic prose are withheld.

New reports inherit private owner-bound artifact retention and joined cleanup.
Normalized names/paths may still be sensitive; neither literal canary tests nor
normalization constitutes general secret detection. Docker daemon/host privileges,
the native allocator and compromised privileged infrastructure remain outside the
unprivileged guest containment claim. See [the security model](docs/security-model.md)
and [ADR-068](docs/adr/ADR-068-m4-runtime-admission.md) for image admission, rollback
and the distinct evidence needed before capability publication.

Coverage HTML and mutation diffs inherited from M3 can contain authorized source
text, including secrets already present in that source. They remain private,
