...
fn aggregate_manifest(
    ...
    for (position, file) in manifest.files.iter().enumerate() {
        ...
        if !supervised.drain_confirmed {
            append_unavailable(&manifest.files[position + 1..], &mut files);
            break;
        }
    }
...
fn append_budget_exhausted(entries: &[ManifestFile], files: &mut Vec<FileSummary>) {
    append_empty(entries, files, FileStatus::BudgetExhausted);
}

fn append_unavailable(entries: &[ManifestFile], files: &mut Vec<FileSummary>) {
    append_empty(entries, files, FileStatus::Unavailable);
}

fn append_empty(entries: &[ManifestFile], files: &mut Vec<FileSummary>, status: FileStatus) {
...
fn supervise_file(index: u32, allowance: Duration) -> Supervised {
    ...
    let drained = receive_drain(&receiver, remaining);
...
fn receive_drain(
    receiver: &Receiver<Result<Vec<u8>, ReadBoundedError>>,
    remaining: Duration,
) -> Result<Result<Vec<u8>, ReadBoundedError>, bool> {
    // After kill+reap the pipe has reached EOF, but the bounded reader still needs
    // a scheduling opportunity to publish it. The outer elapsed clock charges this
    // fixed grace to the supervisor's global wall budget.
    match receiver.recv_timeout(remaining.max(DRAIN_GRACE)) {
        Ok(value) => Ok(value),
...
mod tests {
    ...
    fn aggregation_preserves_prefix_and_marks_budget_or_drain_remainder() {
        ...
        assert!(
            output.files[1..]
                .iter()
                .all(|file| file.status == FileStatus::Unavailable)
        );
    }

    #[test]
    fn drain_grace_accepts_scheduled_eof_and_remains_strictly_bounded() {
        let (sender, receiver) = mpsc::sync_channel(1);
        ...
        assert_eq!(
            receive_drain(&receiver, Duration::ZERO),
            Ok(Ok(vec![1, 2, 3]))
        );
        ...
        let started = Instant::now();
        assert_eq!(receive_drain(&receiver, Duration::ZERO), Err(false));
        assert!(started.elapsed() >= DRAIN_GRACE);
        assert!(started.elapsed() < Duration::from_millis(100));

        let (sender, receiver) = mpsc::sync_channel(1);
        drop(sender);
        assert_eq!(receive_drain(&receiver, Duration::ZERO), Err(true));
    }
