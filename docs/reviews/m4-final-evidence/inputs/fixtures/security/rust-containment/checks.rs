...
pub fn run(phase: &str) {
    process_security();
    sockets();
    let cloud = std::net::SocketAddr::from(([169, 254, 169, 254], 80));
    let error = std::net::TcpStream::connect_timeout(&cloud, std::time::Duration::from_millis(100))
        .expect_err("cloud metadata socket must be denied before any connection");
