...
Other prior dispositions (P3‑1…P3‑10 in `docs/reviews/m4-security-confirmation/disposition.md`, and the bounded composition/contract findings in `docs/reviews/m4-composition-confirmation/review.md` and `docs/reviews/m4-traceability/disposition.md`) I accept as already reviewed and unchanged in this snapshot. I did not re-litigate them.
...
Admission: the altered image is rejected at CLI parse — `crates/mcp-server/src/host_config.rs:146-151` accepts only the three pinned digests, so `main.rs:180` returns exit 2 with `USAGE_ERROR` **before any Docker or guest code runs**; the script asserts empty stdout, exact stderr, no state directory, unchanged product Docker inventory and unchanged project fixture (`:809-816`), with the approved-image EOF control (exit 0) as the positive discriminant (`:820-830`). Receipt `docs/validation/M4-tampered-plugin.json` is `passed`, `script_sha256: 77d7a18f…` matching `inputs.json`, `inputs_unchanged: true`, `cleanup.verified: true`. Failure/repair evidence preserved and honestly labelled at `docs/validation/M4-hardening-attempts/README.md:5-7` and `.../attempt-2/cleanup-repair.json` (`original_cleanup_claim_valid: false`).
