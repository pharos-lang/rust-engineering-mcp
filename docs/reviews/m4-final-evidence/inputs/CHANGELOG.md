...
- Implementados `rust.deny`, `rust.unsafe.scan`, `rust.supply_chain.inspect`,
  ...
  confirmación final de evidencia sigue pendiente; no hay commit de integración,
...
- La [revisión final de código Opus](docs/reviews/m4-final-closure/review.md) no
  ...
  los casos renovados; su confirmación de evidencia permanece abierta antes de
  declarar M4 Done.
...
- Added passive `security-runtime inventory [--json]` for compiled M4 image,
  plugin, nightly and sysroot requirements, without changing doctor v1 or probing
  installations.
