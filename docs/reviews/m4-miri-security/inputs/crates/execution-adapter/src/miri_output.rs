...
fn decode_xml(raw: &[u8], max_bytes: usize) -> Option<String> {
    ...
    while index < raw.len() {
        if raw[index] != b'&' {
            ...
            let plain = std::str::from_utf8(&raw[start..index]).ok()?;
            if plain.chars().any(|character| !valid_xml_char(character)) {
                return None;
            }
            output.push_str(plain);
        } else {
            let relative_end = raw[index..].iter().position(|byte| *byte == b';')?;
            ...
            let character = match entity {
                ...
                _ => {
                    ...
                    let scalar = u32::from_str_radix(text, radix).ok()?;
...
fn next_token(scanner: &mut Scanner<'_>) -> Option<Option<Token>> {
    ...
    if scanner.peek_at(1) == Some(b'?') {
        ...
        return Some(Some(Token::Declaration));
    }
    if scanner.peek_at(1) == Some(b'/') {
        scanner.pos += 2;
...
impl XmlState {
    ...
    fn start(&mut self, tag: Tag, attributes: &[(String, String)]) -> Result<(), MiriParseError> {
        ...
        match tag {
            ...
            Tag::Testcase => {
                if parent != Some(Tag::Testsuite)
                    || self.testcase.is_some()
                    || self.cases.len() >= MAX_TESTCASES
                {
                    return Err(MiriParseError::InvalidJunit);
                }
        ...
        match tag {
            Tag::Testcase => {
                ...
                match testcase.terminal {
                    Some(Terminal::Failure(_)) => suite.actual.failures += 1,
...
fn parse_junit(input: &[u8]) -> Result<ParsedJunit, MiriParseError> {
    ...
    loop {
        let token = next_token(&mut scanner).ok_or(MiriParseError::InvalidJunit)?;
