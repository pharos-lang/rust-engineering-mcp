...
fn command_arguments<'de,D:serde::Deserializer<'de>>(deserializer:D)->Result<Vec<String>,D::Error> {
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        for phase in [
            ...
            let mut null_command=base.clone();null_command[0]["Config"]["Cmd"]=Value::Null;
            assert_eq!(check(&null_command).is_ok(),phase.arguments().is_empty(),"null Cmd for {phase:?}");
            let mut missing_command=base.clone();missing_command[0]["Config"].as_object_mut().ok_or("Config object")?.remove("Cmd");
            assert!(check(&missing_command).is_err(),"missing Cmd for {phase:?}");
