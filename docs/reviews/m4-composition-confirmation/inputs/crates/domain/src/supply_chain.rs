            ...
            && (!self.complete
                || (self.packages_omitted == 0
                    ...
                    && self.audit.as_ref().is_some_and(|audit| {
                        audit.validation_complete && audit.findings_omitted == 0
                    })
                    && self
                        .deny
                        .as_ref()
                        .is_some_and(|deny| deny.complete && deny.findings_omitted == 0)
                    && self.catalog.evidence.as_ref().is_some_and(|evidence| {
                        matches!(
                            evidence.freshness().state(),
                            crate::FreshnessState::Fresh | crate::FreshnessState::Aging
                        )
                    })
                    && self.packages.iter().all(|package| {
                        package.active_features.is_some()
                            && package.declared_features.is_some()
                            && matches!(
                                package.yanked,
                                YankedFact::Yanked
