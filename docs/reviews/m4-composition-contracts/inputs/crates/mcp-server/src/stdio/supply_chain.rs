...
impl SupplyTool {
    pub(super) fn new() -> Result<Self, ErrorData> {
        ...
        let definition=Tool::new(NAME,"Inspect captured dependency facts, checksums, duplicates, features, one RustSec audit, dependency policy and exact-version yanked facts from one authenticated catalog generation. Missing sources and freshness remain explicit. Source locators are withheld; this is no security score or legal approval. Requires MCP Tasks and durable evidence; performs no acquisition.",(*contract.input_schema).clone()).with_raw_output_schema(Arc::clone(&contract.output_schema)).with_annotations(ToolAnnotations::new().read_only(true).destructive(false).idempotent(false).open_world(false));
        Ok(Self {
            definition,
            contract,
            runtime: None,
