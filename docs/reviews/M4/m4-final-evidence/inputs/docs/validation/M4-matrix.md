...
Estado: **Calificación completa; confirmación independiente final pendiente**.
...
ni autorización M5. [Handoff](M4-handoff.md).
...
| --- | --- | --- |
| M4-01 deny | Calificado | [Deny adversarial](M4-deny-adversarial.json), [MCP/rollback](M4-deny-mcp.json), audit único y suppressions exactas. |
| M4-02 unsafe.scan | Calificado | [Siete oráculos nativos](M4-scanner-native.json), origen/omisiones, presupuesto parcial y cleanup. |
| M4-03 Miri | Calificado | [13 clasificaciones y 7 admisión/lifecycle](M4-miri-native.json), warning exacto, panic ordinario y UB separados. |
| M4-04 supply_chain.inspect | Calificado | [Core](M4-core-gate.json): 22 tests security/Supply, seis controles de catálogo firmado; [MCP](M4-tools-mcp.json). |
| M4-05 strict/release | Calificado | Seis tests aplicación GateV2, perfiles nativos, baseline y mutation explícitos; [MCP](M4-tools-mcp.json). |
| M4-06 hardening/cierre | Confirmación final pendiente | [Runtime 19/19](M4-runtime.json), [mapa](M4-hardening-map.md), gates/clientes y revisión de código pasados. |

## Puertas comunes

| Gate | Estado M4 | Evidencia / condición |
| --- | --- | --- |
| G1 contratos | Passed | [Core 19/19](M4-core-gate.json), protocolo 44/44; cinco snapshots nuevos y 23 previos idénticos. |
| G2 autoridad/seguridad | Passed | [Runtime 19/19](M4-runtime.json), [privacidad](M4-privacy-runtime.json), [canarios](M4-output-canaries.json), catálogo y parsers en core/full. |
| G3 lifecycle/budgets | Passed | [Lifecycle final](M4-miri-task-lifecycle.json); [300/300 presupuestos](M4-budgets.json) sobre binario histórico explícito, máximo 17044 ms; [continuidad](M4-benchmark-source-continuity.json) y rutas síncronas actuales verificadas por G4. |
| G4 clientes/native | Passed | [Inspector 2.5.0 y Codex 0.153.0, intento 4](M4-clients.json); 27 tools, cinco M4 reales, Resources, Tasks/negativos en Inspector, sync/modelo en Codex; [runtime 19/19](M4-runtime.json). |
| G5 gates | Passed local | [Core 19/19](M4-core-gate.json), [full 33/33 reanudado](M4-full-gate.json), mismos 987 inputs. Full retiene 27 pasos aprobados y ejecuta los seis restantes tras recuperar E5 local exacto; [driver](M4-full-gate-resume-driver.py), [recuperación](M4-e5-local-recovery.json). Sin código cambiado, descarga ni skip. Sin CI/Sonar remotos ni calificación de host Linux/x86_64; falta linker cross local. |
| G6 rollback | Passed | [Deny MCP](M4-deny-mcp.json): retira admisión al volver a M3, nueva llamada unavailable y mismo artifact privado v1 releído por su owner. |
| G7 inventario | Passed | [Inventario pasivo](M4-runtime-inventory.json), seis binarios/sysroot exactos; [imagen alterada](M4-tampered-plugin.json) rechazada y cleanup verificado. 247 inputs aprovisionados con autorización. |
| G8 review | Confirmación final pendiente | [Sonnet](../reviews/m4-composition-confirmation/review.md), [Gemini](../reviews/m4-traceability/review.md), [Opus final de código](../reviews/m4-final-closure/review.md): sin P0/P1/P2 nuevos. Recibos scanner/Miri ahora renovados; falta confirmar su evidencia. |
| G9 DoR/DoD | Confirmación final pendiente | D19–D22 decididos; decisión final del Technical Owner después de G8. |
...
[Handoff M4](M4-handoff.md) conserva identidad, resultados finales, intentos
