...
fn m4_miri_gateway_classifies_native_oracles() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for name in [
        ...
        assert_eq!(report.clean, name == "clean", "{name}: {report:?}");
        match name {
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    impl ExecutionCancellation for CancelAfter {
        fn is_cancelled(&self) -> bool {
            self.0.elapsed() >= Duration::from_secs(8)
        }
    }
    let state_root = std::path::PathBuf::from("/private/tmp").join(format!(
        "m4-miri-adversarial-{}",
    ...
    for name in [
        ...
        .map_err(|e| format!("{e:?}"))?;
        let cancel = CancelAfter(Instant::now());
        let control: &dyn ExecutionCancellation = if name == "cancel" {
            &cancel
        } else {
            &NeverCancel
        };
        ...
        let result = security_gateway::execute_miri(
            ...
            &source,
            &vendor,
            ExecutionLimits::new_job(if name == "timeout" { 8_000 } else { 60_000 }, 1024 * 1024)
                .ok_or("limits")?,
            control,
        );
        ...
        cases.push(serde_json::json!({"case":name,"error":format!("{error:?}"),"elapsed_ms":started.elapsed().as_millis(),"cleanup_verified":true,"source":resolution_gateway::tree_fingerprint(&source).map_err(|e|format!("{e:?}"))?}));
