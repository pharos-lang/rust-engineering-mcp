...
#[test]
fn unconfigured_supply_vendor_or_policy_is_not_configured_and_incomplete() {
    use rust_engineering_application::supply_chain::{SupplyInputs, SupplyPorts};
    use rust_engineering_domain::supply_chain::SupplyAvailability;
    ...
    for missing_vendor in [true, false] {
        ...
        assert_eq!(
            published.observation.report.deny_availability,
            SupplyAvailability::NotConfigured
        );
        assert!(published.observation.report.deny.is_none());
        assert!(!published.observation.report.complete);
        assert_eq!(engine.deny_calls.load(Ordering::SeqCst), 0);
        assert!(!engine.facts_received_deny.load(Ordering::SeqCst));
        assert_eq!(publisher.calls, 1);
    }
}
...
#[test]
fn supply_rejects_more_than_4096_packages_before_catalog_and_publication() {
    let backend = Backend::default();
    let clock = TestClock::at(100);
    let (mut registry, reference) = open_registry(backend, &clock);
    let template = supply_graph().packages.remove(0);
    let mut graph = supply_graph();
    graph.packages = (0..4097)
        .map(|index| {
            let mut package = template.clone();
            package.name = format!("package-{index}");
            package
        })
        .collect();
    let engine = SupplyEngine::new(SupplyDenyAnswer::Observation(Box::new(clean_deny())), graph);
    let auditor = Auditor::new(clock.clone(), audit_observation(&clock, Some(99)));
    let catalog = SupplyCatalog::available(&clock, Some(99));
    let mut publisher = SupplyTestPublisher::default();
...
#[test]
fn supply_truncates_to_128_packages_and_accounts_for_every_omission() {
    let backend = Backend::default();
    let clock = TestClock::at(100);
    let (mut registry, reference) = open_registry(backend, &clock);
    let template = supply_graph().packages.remove(0);
    let mut graph = supply_graph();
    graph.packages = (0..257)
        .map(|index| {
            let mut package = template.clone();
            package.name = format!("package-{index}");
            package
    ...
    assert_eq!(published.observation.report.packages_total, 257);
    assert_eq!(published.observation.report.packages.len(), 128);
    assert_eq!(published.observation.report.packages_omitted, 129);
    assert!(!published.observation.report.complete);
    assert_eq!(catalog.rows_seen.load(Ordering::SeqCst), 128);
    assert_eq!(publisher.calls, 1);
}
