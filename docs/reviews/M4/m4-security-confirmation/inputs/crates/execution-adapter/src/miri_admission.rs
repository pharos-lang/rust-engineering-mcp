...
pub(super) fn validate_configuration(source: &SourceBundle) -> Result<(), SecurityError> {
    for file in source.files() {
        let parts = file.path().split('/').collect::<Vec<_>>();
        let name = parts.last().copied().unwrap_or_default();
        let parent = parts.iter().rev().nth(1).copied().unwrap_or_default();
        if ((name.eq_ignore_ascii_case("config") || name.eq_ignore_ascii_case("config.toml"))
            && parent.eq_ignore_ascii_case(".cargo"))
            || (name.eq_ignore_ascii_case("nextest.toml") && parent.eq_ignore_ascii_case(".config"))
            || matches!(name, "rust-toolchain" | "rust-toolchain.toml")
        {
...
mod tests {
    use super::*;
    #[test]
    fn captured_project_configuration_is_rejected_before_any_runtime_phase()
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        for path in [
            ...
            let source = SourceBundle::new(vec![
                SourceFile::new(path.into(), vec![]).map_err(|e| format!("{e:?}"))?,
            ])
            .map_err(|e| format!("{e:?}"))?;
            assert_eq!(
                validate_configuration(&source),
        ...
        let source = SourceBundle::new(vec![
            SourceFile::new("Cargo.toml".into(), b"[package]".to_vec())
                .map_err(|e| format!("{e:?}"))?,
        ])
        .map_err(|e| format!("{e:?}"))?;
        assert_eq!(validate_configuration(&source), Ok(()));
        Ok(())
    }
    #[test]
