...
**Done local.** La [matriz M5](matrix.md) es el estado por corte: suite
...
2. Seis selecciones nativas ignoradas, una por vez con `--exact --ignored
   --test-threads=1`: **6/6 aprobadas** ([M5-native-gate.json](native-gate.json)).
3. **PASS** ([M5-clients.json](clients.json), attempt-6).
...
4. `scripts/gate.py core`: **PASS** ([M5-core-gate.json](core-gate.json)).
   `scripts/gate.py full`: **PASS** 38/38 ([M5-full-gate.json](full-gate.json)),
   ...
   ([intento 2](history/gate-attempts/README.md)).
5. Disposición final G1–G9: [Done local](delegation/closure-local-semantics/g1-g9-disposition.md);
...
Las revisiones [vendor](delegation/closure-local-vendor/review.md) y
[semántica](delegation/closure-local-semantics/review.md) detectaron y
...
Claude ni agotamiento de cuota. Sus [intentos](delegation/closure-sonnet5-semantics/attempts.md)
se conservan.
