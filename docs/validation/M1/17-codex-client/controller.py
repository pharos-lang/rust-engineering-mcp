...
def canonical_enc(v): return json.dumps(v,ensure_ascii=True,allow_nan=False,separators=(',',':'),sort_keys=True).encode()
...
def redacted_config(e):
    ...
    return {'mcp_servers':{'rust_engineering':safe},'effective_host_server_enabled':disabled_servers,'effective_feature_values':disabled,'guards':{k:at(e,k) for k in GUARDS},'web_search':e.get('web_search')}
...
def status_inventory(v):
    ...
    return {'server':'rust_engineering','server_version':info.get('version'),'tools':sorted(tools),'canonical_tools_sha256':sha(canonical_enc(tools)),'disabled_host_servers':list(DISABLED_HOST_SERVERS)}
    ...
    def add(self,m):
        ...
        if isinstance(item,dict):
            ...
            if m.get('method')=='rawResponseItem/completed' and typ=='tool_search_output':
                ...
                self.search_outputs.append({'status':item['status'],'canonical_tools_sha256':sha(canonical_enc(item['tools'])),'tool_count':len(item['tools'])})
            if m.get('method')=='rawResponseItem/completed' and typ in ('custom_tool_call','function_call'):
                raise RuntimeError('unexpected_raw_tool_call')
        def usage(x):
            if isinstance(x,dict):
