...
Candidato: rama `ai/m5-performance`, lock con `lancedb 0.31.0` (opción 2a,
`37805f6`). Código Rust idéntico a `a2464c4`; `scripts/` cambió solo en el
harness de clientes y su driver Inspector; `Cargo.toml`/`Cargo.lock` cambian por
la reversión de lancedb. Suite nativa (`37805f6`), clientes (`a19d741`) y
`core`/`full` (`ab4eed9`) miden esas fuentes; el diff entre esos tres commits es
solo documental.
...
| **G4 — fixtures y pruebas** | [`M5-clients.json`](../../M5-clients.json) (attempt-7, lock 0.31.0; attempt-6 midió lo mismo sobre el lock anterior): Inspector 2.5.0 con quince filas y catorce Resources leídas; Claude Code 2.1.267 `claude-sonnet-5` con los dos turnos dirigidos por modelo, IDs emitidos en sesión y contenido de Resource ligado por hash; 74 tests del oráculo Python; intentos 2–5 fallidos preservados con causa | **Demostrado**. `tools_without_a_client_positive = []` |
