...
| `gate.py full` (cierre, Docker/E5/ORT) | **En ejecución** tras el `core` (cadena de cierre); recibo `full-gate.json` al terminar | Sin cambios en el gateway/Docker ni en las suites nativas; el recibo M6 `69a0be14` acredita esas etapas sobre bytes idénticos (`crates/` cambia solo en `mcp-server`: registro estático de definiciones, `contract`, prefijo de 5 descripciones). Se ejecutará al cierre de M8-04/09 |
