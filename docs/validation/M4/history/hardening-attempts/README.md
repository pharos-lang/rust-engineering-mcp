...
- [Full attempt 2](full-attempt-2/receipt.json): first 27 steps passed, then semantic failed on an empty temporary E5 directory. [Recovery](../../e5-local-recovery.json) reverified five exact existing local assets without acquisition. The [driver](../../full-gate-resume-driver.py) executed all six remaining steps through the original gate runner on the identical source inventory; [final full](../../full-gate.json) records 27 retained + six resumed. The original failure is unchanged.
