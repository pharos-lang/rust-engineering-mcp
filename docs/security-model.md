...
`rust.deny` reuses the existing RustSec port and snapshot exactly once. The guest
runs only licenses/bans/sources over authenticated offline Cargo metadata/vendor
bytes. Rules come from host-owned JSON whose expected SHA-256 is supplied by the
host; the client cannot pass a policy path, flags or suppressions. A suppression
requires exact engine/rule/package/source identity, a version requirement, owner,
reason and expiry. It preserves the original finding and cannot repair stale,
missing, invalid or omitted evidence. Expiry is rechecked before publication.
Guest diagnostic messages, labels and notes are discarded; retained rule messages
come from the product's closed mapping. This is policy evidence, not legal advice
or a security certification ([ADR-067](adr/ADR-067-security-policy-and-quality-contracts.md)).

`rust.unsafe.scan` parses source in a process-isolated guest helper. It reports
syntax and byte spans separately for workspace and resolved vendored dependencies.
Comments and strings do not produce findings; cfg-disabled syntax remains syntax,
and macro expansion/generated source is not covered. A file timeout, crash, parse
error or omission remains explicit. The helper enforces per-file/global budgets;
the gateway reserves 25 seconds for controls and cleanup. Scanner absence of
findings never establishes absence of unsafe behavior or UB
([ADR-069](adr/ADR-069-isolated-unsafe-syntax-scanner.md)).

`rust.miri` uses the pinned nightly, prepared sysroot and fixed nextest wrapper;
there is no runtime setup or caller-supplied MIRIFLAGS. Graphs with build scripts,
proc macros or custom harnesses cannot provide authenticated interpreter
diagnostics and are rejected before interpretation. Source Cargo/nextest/toolchain
configuration is rejected. Project stdout/stderr is muted only during test
execution; typed Miri diagnostics and bounded, structurally checked JUnit determine
classification. Empty, skipped, partial, invalid, unsupported and timed-out results
never become clean. A clean observation covers the selected workspace unit and
integration tests only; doctests/examples/benches and universal UB absence are not
claimed. Ten seconds of the job budget remain for JUnit export/revalidation, with
joined cleanup under its separate deadline
([ADR-072](adr/ADR-072-miri-classification-integrity.md)).

Supply-chain reports retain source class and a fingerprint, never the original
source URL, including query strings/fragments/credentials. Lock checksums, duplicate
versions and declared/active features are separate facts. Yanked state requires an
exact version in an authenticated SQLite generation with provenance and freshness;
missing catalog data remains unknown while useful audit/policy facts survive.
Runtime does not download or migrate the catalog
([ADR-071](adr/ADR-071-supply-chain-facts-without-catalog-migration.md)).

`rust.quality.gate.v2` uses a single candidate capture and the shared audit result.
Strict composes standard checks, deny and coverage; release requires a distinct,
explicitly authorized baseline input for SemVer. Baseline may name the same project
reference for a deliberate self-comparison; its captured identity is still bound
separately. Mutation is opt-in with a derived budget checked before effects.
Unavailable, incomplete or omitted required stages cannot pass.

All five tools bound the complete MCP result (including text mirror) to 512 KiB;
reports retain at most 128 findings/packages. Durable reports use the existing
private M3 artifact store, owner checks, TTL, byte quotas and explicit recovery/
pruning. They retain normalized facts, not guest streams. Names and source paths
are still potentially sensitive project metadata; this is not universal secret
scanning. Existing HTML archives remain opaque, never executable resources.
Revocation masks task/resource access, requests cancellation and joins cleanup.
Cleanup uncertainty quarantines the gateway and fails the session; successful
cancellation is not inferred from a protocol notification alone.

Rollback selects the preserved M3 runtime, disables M4 capabilities by removing
their required host configuration, and retains compatible artifact evidence.
