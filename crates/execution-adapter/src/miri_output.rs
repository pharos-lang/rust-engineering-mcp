...
impl JsonEvidence {
    fn merge(&mut self, other: Self) {
        ...
        self.blocks_test_failure |= other.blocks_test_failure;
...
fn analyze_json_line(line: &[u8]) -> JsonEvidence {
    ...
    if level != "error" {
        // This exact diagnostic is produced by the pinned interpreter before
        // an ordinary panic when the captured profile enables optimization.
        // Guest output is muted and native diagnostic producers are rejected.
        const IGNORED_OPTIMIZATION: &str = "Miri does not support optimizations: the opt-level is ignored. The only effect of selecting a Cargo profile that enables optimizations (such as --release) is to apply its remaining settings, such as whether debug assertions and overflow checks are enabled.";
        if level == "warning"
            && message == IGNORED_OPTIMIZATION
            && value.get("code") == Some(&Value::Null)
            && value
                .get("spans")
            ...
            && value
                ...
                .is_some_and(Vec::is_empty)
        {
            evidence.blocks_test_failure = false;
        }
...
fn classify_failed_case(
    ...
    match runner_exit {
        ...
        Some(101)
            if !evidence.blocks_test_failure && !evidence.has_error && !evidence.unexpected =>
        {
...
mod tests {
    ...
    #[test]
    fn pinned_optimization_warning_preserves_panic_but_unknown_diagnostics_do_not()
    -> Result<(), &'static str> {
        let golden = include_str!(
            "../../../fixtures/m4-runtime-oracles/miri-classification/optimized-panic/junit.xml"
        );
        let report = parsed(golden.as_bytes(), 100)?;
        ...
        for changed in [
            ...
            golden.replace("&quot;children&quot;:[]", "&quot;children&quot;:[{}]"),
            golden.replace("&quot;spans&quot;:[]", "&quot;spans&quot;:[{}]"),
        ] {
            assert_ne!(changed, golden);
            let report = parsed(changed.as_bytes(), 100)?;
