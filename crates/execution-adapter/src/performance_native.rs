...
/// docker session (historical receipt: `docs/validation/M5/03-profiling-native.json`
/// at 51fa602e), which is not a receipt of the product path. With the switch armed the
...
fn m5_profile_flamegraph_is_qualified_natively() -> Result<(), Failure> {
    ...
    // which the historical receipt `docs/validation/M5/03-profiling-native.json`
    // (at 51fa602e) declares for this case.
    let execution = {
        let _seccomp = DeniedPerfEventOpen::arm();
