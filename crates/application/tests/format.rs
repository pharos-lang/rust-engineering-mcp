...
struct Formatter<F>(F, Cell<usize>);
impl<F: Fn() -> Result<FormatObservation, InspectionError>> ProjectFormatPort for Formatter<F> {
    fn format(
        &self,
        source: &SourceBundle,
...
fn observation() -> Result<FormatObservation, InspectionError> {
    ...
    Ok(FormatObservation {
        ...
        execution: CheckObservation {
            ...
            stdout: "format difference".into(),
            stderr: "format warning".into(),
...
#[test]
fn format_difference_is_valid_and_publishes_one_log_with_no_raw_streams()
-> Result<(), InspectionError> {
    ...
    let formatter = Formatter(observation, Cell::new(0));
    let checked = registry.format(
        &reference,
        &formatter,
        &mut store,
        (&clock, &clock),
    ...
    assert_eq!(checked.observation.affected_files, ["src/lib.rs"]);
    assert!(
        checked
            .observation
            .diff
            .as_deref()
            .is_some_and(|diff| diff.contains("+fn f() {}"))
    );
    assert_eq!(checked.observation.execution.outcome, CheckOutcome::Failed);
    assert_eq!(checked.observation.execution.exit_code, Some(1));
    assert!(
        checked.observation.execution.stdout.is_empty()
            && checked.observation.execution.stderr.is_empty()
    );
    assert!(checked.observation.execution.stdout_truncated);
...
-> Result<(), InspectionError> {
    for case in [
        Failure::ExpiredDuringFormat,
        Failure::IdentityDuringFormat,
        ...
        let formatter = Formatter(
            || {
                match case {
                    Failure::ExpiredDuringFormat => clock.0.set(10),
                    Failure::IdentityDuringFormat => changed.set(true),
                    _ => {}
                }
                let mut value = observation()?;
                if matches!(case, Failure::Output) {
                    value.execution.stdout = "x".repeat(256 * 1024 + 1);
                }
                Ok(value)
            },
            Cell::new(0),
        ...
        let retired = matches!(
            case,
            Failure::ExpiredDuringFormat | Failure::IdentityDuringFormat
        );
        assert_eq!(
...
#[test]
fn precancel_and_unknown_reference_never_invoke_formatter_or_store() -> Result<(), InspectionError>
{
    let clock = TestClock::default();
    let control = Control::default();
    let formatter = Formatter(observation, Cell::new(0));
...
-> Result<(), InspectionError> {
    ...
    let formatter = Formatter(
        || {
            ...
            observed.execution.stdout = "€".repeat(80_000);
            observed.execution.stderr = "診".repeat(80_000);
            observed.execution.stdout_truncated = false;
            Ok(observed)
        },
        Cell::new(0),
    );
    ...
    assert!(
        checked.observation.execution.stdout_truncated
            && checked.observation.execution.stderr_truncated
    );
    assert!(checked.observation.execution.validation_complete);
    assert_eq!(
...
-> Result<(), InspectionError> {
    for failure in 0..4 {
        ...
        if failure == 0 {
            ...
            assert_eq!(checked.observation.execution.outcome, CheckOutcome::Failed);
            assert!(checked.observation.execution.validation_complete);
            assert!(!checked.observation.diff_omitted);
            assert_eq!(checked.observation.affected_files_omitted, 0);
            assert!(
                checked.observation.execution.stdout.is_empty()
                    && checked.observation.execution.stderr.is_empty()
            );
