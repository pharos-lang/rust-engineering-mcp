...
fn project() -> Result<ProjectClippy, Box<dyn std::error::Error>> {
    ...
    let diagnostics = vec![Diagnostic {
        ...
        code: Some("clippy::needless_return".parse()?),
    ...
    Ok(ProjectClippy {
        ...
        options: ClippySelection::default().try_into()?,
...
fn closed_input_and_validated_options_reject_flags_and_contradictions() -> TestResult {
    ...
    for patch in [
        ...
        json!({"no_default_features":true}),
        json!({"lint_profile":"--force-warn=all"}),
        json!({"lint_profile":null}),
        json!({"lint_profile":42}),
        json!({"features":["x","x"]}),
        json!({"features":["a/../../b"]}),
    ...
    let input=contract.decode(Some(serde_json::from_value(json!({"project_ref":"prj_00000000000000000000000000000001","features":["std","dep/derive"],"all_targets":true,"lint_profile":"pedantic"}))?))?;
...
fn cleanup_uncertainty_outranks_cancellation_and_deadline_without_payload() -> TestResult {
    for signal in [WorkerError::Cancelled, WorkerError::TimedOut] {
        let result: Result<ProjectClippy, _> = joined_result(Joined {
...
#[test]
fn closed_profiles_roundtrip_and_default_matches_native_policy() -> TestResult {
    let contract = Contract::<Input, Output>::new()?;
    let reference = "prj_00000000000000000000000000000001";
    let default = contract
        .decode(Some(serde_json::from_value(
            json!({"project_ref":reference}),
        )?))?
        .options()?;
    assert_eq!(default.lint_profile(), LintProfile::Default);
    let validator =
        jsonschema::validator_for(&serde_json::to_value(contract.output_schema.as_ref())?)?;
    for (name, expected) in [
        ("default", LintProfile::Default),
        ("strict", LintProfile::Strict),
        ("pedantic", LintProfile::Pedantic),
        ("project", LintProfile::Project),
    ] {
        let options = contract
            .decode(Some(serde_json::from_value(
                json!({"project_ref":reference,"lint_profile":name}),
            )?))?
            .options()?;
        assert_eq!(options.lint_profile(), expected);
        let mut project = project()?;
        project.options = options;
        let encoded = contract.encode(output(Ok(project), 1)?)?;
        let mut value = encoded.structured_content.ok_or("output")?;
        assert_eq!(value["data"]["options"]["lint_profile"], name);
        let fields = value["data"]["options"]
            .as_object()
            .ok_or("options")?
            .keys()
            .map(String::as_str)
        ...
        assert_eq!(value["diagnostics"][0]["source"], "clippy");
        assert_eq!(value["diagnostics"][0]["code"], "clippy::needless_return");
        value["data"]["options"]["lint_profile"] = json!("other");
        assert!(!validator.is_valid(&value));
    }
    for error in [
