//! One admitted Clippy run owns capture, execution, normalization and log publication.
#[allow(dead_code)]
mod schemas;
use super::{
...
use rust_engineering_domain::{
    CheckOutcome, ClippyOptions, ClippySelection, Clock, Diagnostic, Evidence,
    ExecutionTermination, LintProfile, OperationalErrorCode, ProjectClippy, ProjectRef,
    RuntimeIdentity, ToolStatus, UnixSeconds,
};
...
pub(super) const NAME: &str = "rust.clippy";
...
struct Input {
    ...
    all_targets: bool,
    #[serde(default)]
    #[schemars(with = "schemas::LintProfile")]
    lint_profile: LintProfile,
}
impl Input {
    fn options(&self) -> Result<ClippyOptions, ErrorData> {
        ClippySelection {
...
struct Data {
    ...
    semantics: Semantics,
    #[schemars(with = "schemas::ClippyOptions")]
...
fn operational(code: OperationalErrorCode) -> (Outcome, &'static str) {
    let (code, message, unavailable) = match code {
        ...
        OperationalErrorCode::CommandTimeout => {
            (Code::CommandTimeout, "Clippy exceeded its deadline", false)
        }
        OperationalErrorCode::SandboxDenied => (
            Code::SandboxDenied,
            "Host runtime policy, failed calibration or current capacity denied Clippy",
            false,
        ),
        ...
        OperationalErrorCode::UnsupportedPlatform => (
            ...
            "Secure Clippy is unavailable on this platform",
            true,
        ),
        OperationalErrorCode::OutputLimitExceeded => (
            Code::OutputLimitExceeded,
            "Clippy evidence could not be retained within the output budget",
            false,
        ),
    };
    (
...
fn output(
    result: Result<ProjectClippy, InspectionError>,
    duration_ms: u64,
    ...
    let (outcome, summary) = match result {
            ...
            if observation.termination == ExecutionTermination::TimedOut {
                (
                    Outcome::Blocked {
                        ...
                        error_message: "Clippy exceeded its deadline",
                        data: Some(data),
                    },
                    "Clippy timed out; partial evidence is retained",
                )
            } else {
                match observation.outcome {
                    ...
                    CheckOutcome::Passed => (
                        ...
                        if has_log {
                            "Captured project passed Cargo Clippy in the approved offline runtime"
                        } else {
                            "Cargo Clippy passed; log retention capacity exhausted"
                        },
                    ),
                    CheckOutcome::Failed => (
                        Outcome::Failed {
                        ...
                        if has_log {
                            "Cargo Clippy reported compilation or lint failure"
                        } else {
        ...
        Err(
            ...
            "Clippy cancelled after worker completion",
        ),
        ...
        ) => return Err(ErrorData::internal_error("Clippy failed", None)),
    };
    Ok(Output {
        outcome,
        summary,
...
impl ClippyTool {
    pub(super) fn new(
        ...
        let definition=Tool::new(NAME,"Lint captured Rust source with Clippy using host-approved offline Cargo. Can execute build scripts and proc macros inside the calibrated sandbox. Accepts bounded closed selections and default, strict, pedantic or project lint profiles. Default and project honor native captured lint policy; strict denies warnings and pedantic opts into warning-level pedantic lints. Source allows may suppress lints. Returns normalized diagnostics and an ephemeral owner-authorized log Resource. Project code may write the diagnostic stream; normalization does not authenticate its origin. Requires a live project_ref and completed discovery; installs nothing.",(*contract.input_schema).clone())
    ...
    pub(super) async fn call(
        ...
        if bootstrap {
            let message = "Clippy requires completed discovery; retry with a new request ID";
